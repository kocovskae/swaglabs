...
test('Verify redirection to Checkout page after clicking Checkout button', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
...
test('Verify that the Checkout form is displayed', async ({ inventoryPage, cartPage, checkoutPage }) => {
    await inventoryPage.addToCart(0);
    await cartPage?.clickOnCartIcon();
    await cartPage?.verifyCartPageIsDisplayed();
    await cartPage?.verifyCheckoutButtonIsDisplayed();
...
test('Verify that the Cancel and Continue buttons are displayed on the Checkout form', async ({ inventoryPage, cartPage, checkoutPage }) => {
    await inventoryPage.addToCart(0);
    await cartPage?.clickOnCartIcon();
    await cartPage?.verifyCartPageIsDisplayed();
    await cartPage?.verifyCheckoutButtonIsDisplayed();
...
test('Verify that the Checkout Overview page displays product and order details', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
...
test('Verify that clicking Cancel on the Checkout Overview redirects the user to the Products page', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
    await inventoryPage.addToCart(3);
    await cartPage?.clickOnCartIcon();
    await cartPage?.verifyCartPageIsDisplayed();
    await cartPage?.verifyCheckoutButtonIsDisplayed();
...
test('Verify that completing checkout shows a successful payment confirmation', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
    await inventoryPage.addToCart(3);
    await cartPage?.clickOnCartIcon();
    await cartPage?.verifyCartPageIsDisplayed();
    await cartPage?.verifyCheckoutButtonIsDisplayed();
...
test('Verify that the user can successfully complete the checkout process', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
    await inventoryPage.addToCart(0);
    await cartPage?.clickOnCartIcon();
    await cartPage?.verifyCartPageIsDisplayed();
    await cartPage?.verifyCheckoutButtonIsDisplayed();
...
test('Verify error messages for invalid inputs in Checkout form', async ({ page, inventoryPage, cartPage, checkoutPage }) => {
